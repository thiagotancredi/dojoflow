...
class StudentsMenuHandler:
    ...
    async def process_callback(  # noqa: PLR0911
        self,
        chat_id: int,
    ...
    async def process_student_due_day_message(
        ...
        await state_service.update_student_creation_context(
            state_id=state_id,
            next_step=TelegramStep.WAITING_STUDENT_IS_EXEMPT,
            context_data=updated_context_data,
        )

        await self.telegram_service.send_message(
            chat_id=chat_id,
            text='Esse aluno é isento de mensalidade?',
            reply_markup=yes_no_required_reply_markup(
                yes_callback_data='students:create:exempt:yes',
                no_callback_data='students:create:exempt:no',
            ),
        )

        return {'status': 'waiting_student_is_exempt'}
    ...
    async def _process_confirmation(
        ...
        await self.telegram_service.send_message(
            ...
            text=(
                ...
                f'Nome: {student.name}\n\n'
                'O aluno já foi vinculado à modalidade escolhida.'
            ),
            reply_markup=students_menu_reply_markup(),
        )
    ...
    def _build_student_summary(
        ...
        is_exempt = 'Sim' if context_data['is_exempt'] else 'Não'
        ...
        return (
            ...
            'Data de nascimento: '
            f'{context_data.get("birth_date") or "Não informado"}\n'
